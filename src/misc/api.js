...
const authenticate = async () => {
  let processReturnBool;
  const response = await fetchResource(`https://api.gotinder.com/v2/auth/login/facebook`, {
    token: process.env.FACEBOOK_API_TOKEN
  });
  const token = get(response, 'data.api_token');
  if (token) {
    headers['X-Auth-Token'] = token;
    localStorage.setItem(`TinderWeb/APIToken`);
    processReturnBool = true
  } else {
    processReturnBool = false
    throw new Error('User base case authentication failed');
  }
  return processReturnBool
};
